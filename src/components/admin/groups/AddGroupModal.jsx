// src/components/admin/groups/AddGroupModal.jsx
...
export default function AddGroupModal({
  ...
  const [icon, setIcon] = useState(ICON_SET[0]?.id || "globe");
  const [color, setColor] = useState(COLOR_SET[0] || "bg-amber-500");
  ...
  return (
    <div className="fixed inset-0 bg-black/40 p-4 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-stone-800 p-6 rounded-2xl w-full max-w-md border border-stone-200 dark:border-stone-700 shadow-xl space-y-5">
        <h3 className="text-lg font-bold text-stone-800 dark:text-stone-100">Neue Gruppe</h3>
        ...
        <div>
          ...
          <input
            ...
            placeholder="z. B. Luft, Wolken, Kreativ ..."
          />
        </div>

        {/* Icons (Grid, ohne Labels) */}
        <div>
          <label className="text-xs text-stone-500 dark:text-stone-400 uppercase font-semibold">
            Icon
          </label>
