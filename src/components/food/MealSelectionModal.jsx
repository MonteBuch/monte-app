  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center">
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl overflow-hidden">
          ...
          {options.map((opt, index) => (
            ...
            >
              ...
              <button
                ...
                className="p-1 rounded-full text-stone-300 group-hover:text-red-500"
              >
                <Trash2 size={16} />
              </button>
            </div>
        ...
        {/* ADD NEW OPTION */}
        <div className="px-5 py-4 border-t border-stone-200 flex items-center gap-2">
          <input
            value={newEntry}
            onChange={(e) => setNewEntry(e.target.value)}
            placeholder="Neuen Eintrag hinzufügen…"
            className="flex-1 p-2 rounded-2xl border border-stone-300 bg-stone-50 text-sm"
          />
          ...
          <button
            ...
            className="w-9 h-9 rounded-2xl bg-stone-900 text-white flex items-center justify-center hover:bg-stone-800"
          >
            <Plus size={16} />
          </button>
        </div>
